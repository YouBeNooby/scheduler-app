...
# Define dynamic browser configuration maps
...
if st.session_state.court_config is not None:
    ...
    sport_emoji_map = {
        "badminton": "🏸", "tennis": "🎾", "football": "⚽", "soccer": "⚽",
        "basketball": "🏀", "squash": "🏓", "swimming": "🏊", "cricket": "🏏", "volleyball": "🏐"
    }
...
if not st.session_state.logged_in:
    ...
    with st.form("auth_form", clear_on_submit=True):
        ...
        submit_auth = st.form_submit_button(menu)
            ...
            if not username or not password:
                ...
                if username.lower() == "admin":
                    ...
                    if not existing_df.empty:
                        ...
                        try:
                            with conn.session as session:
                                session.execute(text("INSERT INTO tennis_users (username, password) VALUES (:u, :p)"), {"u": username, "p": make_hashes(password)})
                                session.commit()
                ...
                if not user_df.empty:
                    ...
                    if hashed_input == stored_password:
                        ...
                        if remember_me and cookie_manager:
                            ...
                            with conn.session as session:
                                session.execute(text("INSERT INTO tennis_sessions (token, username, created_at) VALUES (:t, :u, :c)"), {"t": secure_token, "u": username, "c": current_timestamp})
                                session.commit()
                            ...
                            try:
                                cookie_manager.set(cookie="scheduler_vault_tokens", val=new_cookie_val, expires_at=datetime.now() + pd.Timedelta(days=30))
                            except Exception: pass
                        st.rerun()
                    else:
...
# ---------------- MAIN APP (AUTHENTICATED SESSIONS) ---------------- #
    ...
    # ---------------- INTERFACE ARCHITECTURE: GLOBAL ACCOUNT SIDEBAR ---------------- #
    with st.sidebar:
                ...
                if submit_change:
                    if not current_password or not new_password or not confirm_password:
                        ...
                        if not user_data_df.empty and make_hashes(current_password) == user_data_df.iloc[0]["password"]:
                            ...
                            with conn.session as session:
                                session.execute(text("UPDATE tennis_users SET password=:p WHERE username=:u"), {"p": new_hashed, "u": st.session_state.username})
                                session.commit()
            ...
            if cookie_manager:
                ...
                if vault_cookie:
                    ...
                    with conn.session as session:
                        for t in tokens:
                            if t.strip():
                                session.execute(text("DELETE FROM tennis_sessions WHERE token=:t"), {"t": t.strip()})
                        session.commit()
    ...
    if st.session_state.username == "admin":
        ...
        with admin_tab1:
            ...
            with st.form("admin_access_code_form", clear_on_submit=True):
                ...
                tz_input = st.selectbox("Facility Local Timezone", options=all_tz_options, index=all_tz_options.index("Asia/Kolkata") if "Asia/Kolkata" in all_tz_options else 0)
                ...
                if submit_config:
                    if not sport_input or not code_input:
                        ...
                        try:
                            ...
                            with conn.session as session:
                                session.execute(text("INSERT INTO tennis_court_configurations (sport, court_count, access_code, timezone, created_at) VALUES (:s, :cc, :ac, :tz, :cat)"),
        ...
        with admin_tab3:
            ...
            if not all_users_df.empty:
                for _, row in all_users_df.iterrows():
                    ...
                    if user_to_manage == "admin": continue
            ...
            if submit_gate:
                ...
                if not match_df.empty:
                    ...
                    if remember_code and cookie_manager:
                        try:
                            cookie_manager.set(cookie="stadium_access_code", val=entered_code, expires_at=datetime.now() + pd.Timedelta(days=30))
                        except Exception: pass
    ...
    # HIGHLY DETAILED DYNAMIC COURT SUB-TABS
    ...
    if not user_bookings_df.empty:
        for _, row in user_bookings_df.iterrows():
            ...
            with col1:
                st.write(f"📌 {row['booking_date']} at {row['slot']} (Court #{row.get('court_number', 1)} — {row.get('sport', 'Badminton')})")
            with col2:
